...
test("a new driver is added to the shared roster, with who uploaded it", async () => {
  const db = fakeDb();
  const result = await recordChange(db, change("driver", driverData()), { actor: "raul", groups: ["Admin"], now });
...
test("an older edit arriving late does not overwrite a newer one; it is kept as superseded", async () => {
  ...
  const result = await recordChange(db, change("driver", driverData({ name: "Old Name" })), { actor: "raul", groups: ["Admin"], now });
...
test("a new vehicle is inserted carrying the device's id", async () => {
  const db = fakeDb();
  const result = await recordChange(db, change("vehicle", vehicleData()), { actor: "raul", groups: ["Admin"], now });
  assert.equal(result.outcome, "applied");
...
test("an edited vehicle is found by the barcode it had, so a barcode change moves it rather than adding a second car", async () => {
  ...
  const result = await recordChange(db, change("vehicle", vehicleData({ id: "veh-005", previousBarcode: "G0005", assignedBarcode: "G0105" })), { actor: "raul", groups: ["Admin"], now });
  assert.equal(result.outcome, "applied");
...
test("a barcode already on another vehicle is refused", async () => {
  ...
  await assert.rejects(() => recordChange(db, change("vehicle", vehicleData({ assignedBarcode: "G0006" })), { actor: "raul", groups: ["Admin"], now }), (error) => error.status === 409 && /already belongs to another vehicle/.test(error.message));
});
...
test("removing a vehicle records when and by whom", async () => {
  ...
  await recordChange(db, change("vehicle", vehicleData({ active: false })), { actor: "raul", groups: ["Admin"], now });
  const update = wrote(db, "update vehicles")[0];
...
test("an authorization for a driver the shared roster does not know is refused", async () => {
  const db = fakeDb();
  await assert.rejects(() => recordChange(db, change("authorization", authData()), { groups: ["Admin"], now }), (error) => error.status === 422 && /not in the shared roster/.test(error.message));
});
...
test("a new authorization is stored and replaces an earlier active one the device never saw", async () => {
  ...
  const result = await recordChange(db, change("authorization", authData()), { actor: "raul", groups: ["Admin"], now });
  assert.equal(result.outcome, "applied");
  const insert = wrote(db, "insert into authorizations")[0];
  assert.equal(insert.params.status, "active");
  assert.equal(insert.params.role, "Admin", "the signed-in person's role, not what the device claimed");
...
test("a gate approval takes the approver's rank from the shared list, and a Scanner badge is refused", async () => {
  ...
  await recordChange(fleet, change("authorization", authData({ approverBadge: "s2040", authorizedRole: "Admin" })), { actor: "raul", groups: ["Admin"], now });
  assert.equal(wrote(fleet, "insert into authorizations")[0].params.role, "Fleet Lead", "the device's claim of Admin is not believed");
  await assert.rejects(() => recordChange(lookup("Scanner"), change("authorization", authData({ approverBadge: "S3090" })), { actor: "raul", groups: ["Admin"], now }), (error) => error.status === 403);
});
...
test("revoking ends the authorization at the time it was revoked, and every other active one for that driver", async () => {
  ...
  const result = await recordChange(db, change("authorization", authData({ status: "revoked", revokedAt, revokedBy: "Supervisor Console", revocationReason: "Manual revocation" }), { occurredAt: revokedAt }), { actor: "raul", groups: ["Admin"], now });
  assert.equal(result.outcome, "applied");
...
test("an ended authorization is never revived", async () => {
  ...
  const result = await recordChange(db, change("authorization", authData({ status: "active" })), { actor: "raul", groups: ["Admin"], now });
  assert.equal(result.outcome, "superseded");
...
test("a revocation time is never before the authorization began, nor after it arrived", async () => {
  ...
  await recordChange(early, change("authorization", authData({ status: "revoked", revokedAt: "2026-09-18T00:00:00.000Z" })), { actor: "raul", groups: ["Admin"], now });
  ...
  await recordChange(late, change("authorization", authData({ status: "revoked", revokedAt: "2026-09-19T20:00:00.000Z" }), { occurredAt: "2026-09-19T14:00:00.000Z" }), { actor: "raul", groups: ["Admin"], now });
