...
// CR-V14 item 3: employee numbers may contain letters, so the canonical form is no longer always
// E####. Numeric IDs still fold onto E####, and anything with letters is kept as entered.
assert.match(app, /return legacyNumeric \? `E\$\{legacyNumeric\[1\]\}` : source;/, "numeric employee IDs must still canonicalise to E####");
...
assert.match(worker, /lot-watch-gateflow-v0\.8-shared-records-static/, "cache version must change for the CR-V17 shared-records build");
